...
XXXXXXXXXXXXXXXXx
XXXXXXXXXXXXXXXXx TODO: Change here:
XXXXXXXXXXXXXXXXx
...
```{r}
data_surv %>% filter(type == 'switcher')
```
...
```{r}
#data_static <- read_rds('../../temp/data_author_static.rds')
```

Matching strategy:

* match persons by year (exact) on:
  * current seniority (exact)
  * current concept (approximate)
  * current paper_n, citation / novelty rank (approximate)
  * current institution citation / novelty rank (approximate)
...
TODO: Check if the factor is right
...
data_did %<>%
  group_by(pair_id) %>%
  mutate(year_transit = max(year_transit, na.rm = TRUE),
...
fit_did_2 <- lm(citation_fw ~ (type + transited)^2 + (transited + transited_t)^2 + year + seniority + year + concepts + transited_t, 
                data = data_did)

fit_did_3 <- lm(novelty_fw ~ (type + transited)^2 + year + seniority + year + concepts, 
                data = data_did)

fit_did_4 <- lm(novelty_fw ~ (type + transited)^2 + (transited + transited_t)^2 + year + seniority + year + concepts + transited_t, 
                data = data_did)
...
```{r}
stargazer(fit_did_1, fit_did_2, fit_did_3, fit_did_4, type = 'text',
          omit = c('concepts', 'year', 'Constant'))
```


```{r}
# stargazer(fit_did_0, fit_did_1, fit_did_2, fit_did_3, fit_did_4, type = 'latex',
#          omit = c('concepts', 'year', 'Constant'))
```
