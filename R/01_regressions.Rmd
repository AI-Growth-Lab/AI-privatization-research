...
data_match <- data %>%
  ...
  select(AID, type, year, aff_id, seniority, paper_n, author_mean, oa_mean, concepts, citation_fw, novelty_fw) %>%
...
match_out <-  data_match %>% 
  # Matching
  matchit(type ~ aff_id + paper_n + author_mean + oa_mean + concepts + citation_fw + novelty_fw, exact = c('year', 'seniority'),
...
matched_pairs %<>%
  left_join(data %>% filter(switch == TRUE) %>% select(author_id, time) %>% distinct(author_id, .keep_all = TRUE), by = c('orig_AID' = 'AIDd')) %>%
  mutate(switch = TRUE) %>%
  drop_na() 
```
