...
figa = ggplot(df, aes(x = year)) + 
  geom_line(linewidth=1.25,aes(y = ai_publications, color='ai_publications')) + 
  geom_line(linewidth=1.25,aes(y = share*6768480, color='ai_share')) +
...
figb = ggplot(df, aes(x = year)) + 
  geom_line(size=1.25,aes(y = share_solo_authors, color='share_solo_authors')) + 
  geom_line(size=1.25,aes(y = share_companies*3, color='share_companies')) +
  scale_color_manual(name = "Group",
  ...
  scale_y_continuous(
    name = "Share solo aut",
    sec.axis = sec_axis(~./3, name="Share comp")
  )
...
figb = ggplot(df, aes(x = year)) + 
  geom_line(size=1.25,aes(y = share_solo_authors, color='share_solo_authors')) + 
  geom_line(size=1.25,aes(y = share_companies, color='share_companies')) +
  scale_color_manual(name = "Group",
                     values = c( "share_solo_authors" = "#1f77b4", "share_companies" = "#ff7f0e"),
                     labels = c( "Comp participation","Share solo aut")) +
  labs(x = "Year",
...
figc = ggplot(df, aes(x = year)) + 
  geom_line(size=1.25,aes(y = education_company, color='education_company')) + 
  geom_line(size=1.25,aes(y = company_education, color='company_education')) +
  scale_color_manual(name = "Group",
                     values = c( "education_company" = "blue", "company_education" = "red"),
                     labels = c( "Comp to edu","Edu to comp")) +
  labs(x = "Year",
         y = "Number of researcher",
         color = "Legend") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1, size = 6),
      axis.text.y = element_text(size = 6),
      axis.title.x = element_text(size = 6),
...
figure <- ggarrange(figb, figb, figc, figd,
                    labels = c("A", "B", "C","D"),
                    ncol = 2, nrow = 2)
figure
...
ggarrange(
  ...
  ggarrange(figb, figb,figc, ncol = 3, labels = c("A","B", "C")),
  figd,
  nrow = 2, 
